...
export function CreateHouseScreen({ onSuccess, onBack }) {
  ...
  async function handleCreate() {
    if (!houseName.trim() || !adminName.trim() || !adminEmail.trim()) return;
    setLoading(true);
    setError(null);
    try {
      const session = await api.createHouse(houseName.trim(), adminName.trim(), adminEmail.trim());
      onSuccess(session);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col items-center justify-center p-6 gap-4">
      ...
      <div className="w-full max-w-[360px] flex flex-col gap-3">
        <div>
          ...
          <input
            ...
            placeholder="es. Via Risorgimento"
            className={inputCls}
          />
        </div>
        <div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="es. giada@example.com"
            className={inputCls}
          />
        </div>
      </div>
      ...
      {error && (
        <p className="text-[.82rem] text-red-500 text-center max-w-[320px]">{error}</p>
      )}

      <button
        onClick={handleCreate}
        disabled={loading || !houseName.trim() || !adminName.trim() || !adminEmail.trim()}
